...
def read_exr_bgr2rgb(path):
    ...
    if str(path).endswith('.png'):
        ...
        rgb = np.array(img)
        # GameIR pngs are RGBA or RGB. Return as HWC numpy array
        return rgb
    else:
...
def read_exr_bgr2rgb_from_bytes(content):
    import io
    from PIL import Image
    try:
        ...
        import cv2
        img_np = np.frombuffer(content, np.uint8).copy()
        ...
        if len(bgr.shape) == 3 and bgr.shape[2] == 3:
            ...
            rgb = bgr # Grayscale or other
        return rgb

def load_image_from_bytes(content, format='rgb'):
    rgb = read_exr_bgr2rgb_from_bytes(content)[:,:,:3] / 255.
    if format == 'rgb':
        return rgb
    elif format == 'ycbcr':
...
def load_exr_from_bytes(content, buffer_type):
    image = read_exr_bgr2rgb_from_bytes(content)
...
def imgs2tensor(imgs):
  ...
  return torch.stack([torch.from_numpy(img).permute(2, 0, 1).float() for img in clean_imgs], dim=0)
...
def load_img_seq(path_list, image_type = 'Image', format = 'rgb'):
    if image_type == 'Image':
      ...
      imgs = [load_image(path, format)[:,:,0:1] for path in path_list]
      return imgs2tensor(imgs)
    else:
