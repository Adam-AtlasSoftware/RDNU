...
class CTR(nn.Module):
    ...
    def forward(self, x, depth, pre_h=None, flow=None):
        ...
        c_q = q1.shape[1] // self.head
        c_k = k.shape[1] // self.head
        
        q1 = q1.view(b, self.head, c_q, h * w)
        k = k.view(b, self.head, c_k, h * w).transpose(-2, -1)
        v = v.view(b, self.head, c_k, h * w)
        ...
        out = out.view(b, self.head * c_q, h, w)
...
class CycleFC(nn.Module):
    def __init__(
        ...
        # Optimized Depthwise Shift
        pad_y = self.kernel_size[0] // 2
        pad_x = self.kernel_size[1] // 2
        
        shift_weight = torch.zeros(in_channels, 1, self.kernel_size[0], self.kernel_size[1])
        start_idx = (self.kernel_size[0] * self.kernel_size[1]) // 2
        assert self.kernel_size[0] == 1 or self.kernel_size[1] == 1, self.kernel_size
        for i in range(self.in_channels):
            if self.kernel_size[0] == 1:
                dy = 0
                dx = (i + start_idx) % self.kernel_size[1] - (self.kernel_size[1] // 2)
            else:
                dy = (i + start_idx) % self.kernel_size[0] - (self.kernel_size[0] // 2)
                dx = 0
            shift_weight[i, 0, dy + pad_y, dx + pad_x] = 1.0
            
        self.register_buffer('shift_weight', shift_weight)
    ...
    def forward(self, input):
        ...
        """
        # Optimized spatial shift using depthwise convolution
        pad_y = self.kernel_size[0] // 2
        pad_x = self.kernel_size[1] // 2
        shifted_x = F.conv2d(input, self.shift_weight, padding=(pad_y, pad_x), groups=self.in_channels)
