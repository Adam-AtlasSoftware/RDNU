...
class RDG(nn.Module):
    ...
    def forward(self, input_data):
        ...
        motions = input_data.get('Motion')
        if motions is None:
            raise ValueError("Motion vectors are required but missing from dataset.")
        ...
        normal = input_data.get('Normal')
        if normal is None:
            normal = torch.zeros(images.shape[0], images.shape[1], 3, images.shape[3], images.shape[4], device=images.device)

        brdf = input_data.get('BRDF')
        if brdf is None:
            brdf = torch.zeros(images.shape[0], images.shape[1], 3, images.shape[3], images.shape[4], device=images.device)

        gbuffers = torch.cat([normal, brdf], dim = 2)
