// Generated by runtime/tools/port_nss_shaders.py from Arm's ffx_nss_common_glsl.h (MIT).
...
ConstantBuffer<cbNSS_t> cbNSS : RDNU_REG_B(NSS_BIND_CB_NSS);
...
Texture2D<float4> r_input_color_jittered : RDNU_REG_T(NSS_BIND_SRV_INPUT_COLOR_JITTERED);
...
Texture2D<float4> r_input_motion_vectors : RDNU_REG_T(NSS_BIND_SRV_INPUT_MOTION_VECTORS);
...
Texture2D<float4> r_prev_upscaled_color : RDNU_REG_T(NSS_BIND_SRV_HISTORY_UPSCALED_COLOR);
...
Texture2D<float4> r_input_depth : RDNU_REG_T(NSS_BIND_SRV_INPUT_DEPTH);
...
RWTexture2D<uint> rw_reconstructed_prev_depth : RDNU_REG_U(NSS_BIND_UAV_DEPTH_TM1);
#endif
...
#define MAX_FP16      65504.0h
#define EPS           1e-7h
#define MIN_SUMW_HALF 6.1035156e-5h
...
half3 Tonemap(half3 x)
{
    ...
    half3 xc = clamp(x, ((half3)(0.0h)), ((half3)(MAX_FP16)));
...
half3 InverseTonemap(half3 x)
{
    ...
    // http://graphicrants.blogspot.com/2013/12/tone-mapping.html
    x = clamp(x, ((half3)(0.0h)), ((half3)(1.0h - EPS)));
    return x * rcp(((half3)(1.0h)) - max(max(x.r, x.g), x.b));
}
...
{
    return int8_t(clamp(round(f * quant_params.x + quant_params.y), half(-128.0h), half(127.0h)));
}

int8_t2 Quantize(half2 f, half2 quant_params)
{
    return ((int8_t2)(clamp(round(f * quant_params.x + quant_params.y), ((half2)(-128.0h)), ((half2)(127.0h)))));
}

int8_t3 Quantize(half3 f, half2 quant_params)
{
    return ((int8_t3)(clamp(round(f * quant_params.x + quant_params.y), ((half3)(-128.0h)), ((half3)(127.0h)))));
}

int8_t4 Quantize(half4 f, half2 quant_params)
{
    return ((int8_t4)(clamp(round(f * quant_params.x + quant_params.y), ((half4)(-128.0h)), ((half4)(127.0h)))));
}

half3 SafeColour(half3 x)
{
    return clamp(x, ((half3)(0.0h)), ((half3)(MAX_FP16)));
}
...
RWTexture2D<float4> rw_debug_views : RDNU_REG_U(NSS_BIND_UAV_DEBUG_VIEWS);
