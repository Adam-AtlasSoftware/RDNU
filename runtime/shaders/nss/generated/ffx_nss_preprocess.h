// Generated by runtime/tools/port_nss_shaders.py from Arm's ffx_nss_preprocess.h (MIT).
...
Texture2D<float4> r_input_nearest_depth_coord_tm1 : RDNU_REG_T(NSS_BIND_SRV_NEAREST_DEPTH_COORD_TM1);
...
Texture2D<float4> r_temporal_feedback_tensor : RDNU_REG_T(NSS_BIND_SRV_FEEDBACK_TENSOR);
...
Texture2D<float4> r_prev_luma_deriv : RDNU_REG_T(NSS_BIND_SRV_LUMA_DERIV_TM1);
...
Texture2D<uint4> r_prev_depth : RDNU_REG_T(NSS_BIND_SRV_INPUT_DEPTH_TM1);
...
Texture2D<float4> r_disocclusion_mask_lq : RDNU_REG_T(NSS_BIND_SRV_DISOCCLUSION_MASK_LQ);
...
RWTexture2D<float4> rw_luma_deriv : RDNU_REG_U(NSS_BIND_UAV_LUMA_DERIV);
#else
RWTexture2D<float4> rw_luma_deriv : RDNU_REG_U(NSS_BIND_UAV_LUMA_DERIV);
...
RWTexture2D<float4> rw_nearest_depth_coord_out : RDNU_REG_U(NSS_BIND_UAV_NEAREST_DEPTH_COORD);
#else
RWTexture2D<float4> rw_nearest_depth_coord_out : RDNU_REG_U(NSS_BIND_UAV_NEAREST_DEPTH_COORD);
#endif
...
static const float kDepthScale        = 2147483647.0;
static const float kInvDepthScale     = 1.0 / kDepthScale;
static const float kMotionThreshold   = 0.1;
static const float kMotionThresholdSq = kMotionThreshold * kMotionThreshold;
...
static const half2 kTemporalFeedbackQuant = half2(0.49999999813735485, -1.0);

#if !NSS_YCOCG_LUMA_DERIVATIVE
static const half kDerivativeDisThresh = 0.01h;
static const half kDerivMin            = 0.05h;
static const half kDerivMax            = 0.3h;
static const half kDerivAlpha          = 0.1h;
#endif  // !NSS_YCOCG_LUMA_DERIVATIVE
...
half3 LoadColourForDerivativeAtResolvedPixel(int32_t2 sample_coord)
{
    ...
    c       = max(c * half(Exposure().x), ((half3)(0.0h)));
    return sqrt(c);
}
...
half4 WarpFeedback(float2 uv, half disocclusion_mask)
{
    ...
    return mix(feedback, ((half4)(0.0h)), half(disocclusion_mask > 0.01h));
}
...
static const half4 kDerivativeStorageDecodeScale = half4(4.0h, 8.0h, 8.0h, 0.5h);
static const half4 kDerivativeStorageDecodeBias  = half4(4.0h, 0.0h, 0.0h, 0.5h);
static const half4 kDerivativeStorageEncodeScale = half4(0.25h, 0.125h, 0.125h, 2.0h);
static const half4 kDerivativeStorageEncodeBias  = half4(-1.0h, 0.0h, 0.0h, -1.0h);
static const half4 kDerivativeStorageZeroState   = half4(-1.0h, 0.0h, 0.0h, -1.0h);

half4 RGBToYCoCgDerivativeState(half3 rgb)
{
    half co = rgb.r - rgb.b;
    half t  = rgb.b + co * 0.5h;
    half cg = rgb.g - t;
    half y  = t + cg * 0.5h;
    return half4(y, co, cg, 0.0h);
}
...
{
    const half  derivative_dis_thresh       = 0.01h;
    const half4 delta_weight                = half4(1.0h, 1.5625h, 1.5625h, 0.0h);
    const half  recall_floor                = 0.065h;
    const half  recall_ceil                 = 0.420h;
    const half  excursion_floor             = 0.025h;
    const half  excursion_ceil              = 0.160h;
    const half  mean_gate_floor             = 0.070h;
    const half  mean_gate_ceil              = 0.230h;
    const half  sustain_cold_floor          = 0.177h;
    const half  sustain_cold_ceil           = 0.330h;
    const half  sustain_hot_floor           = 0.157h;
    const half  sustain_hot_ceil            = 0.305h;
    const half  sustain_hysteresis_floor    = 0.110h;
    const half  sustain_hysteresis_ceil     = 0.210h;
    const half  sustain_support_alpha       = 0.30h;
    const half  hot_hold_floor              = 0.180h;
    const half  hot_hold_ceil               = 0.280h;
    const half  sustain_min_hot_hold        = 0.12h;
    const half  decay_min_hot_gate          = 0.50h;
    const half  sustain_strength            = 0.80h;
    const half  instability_rise_alpha_min  = 0.08h;
    const half  instability_rise_alpha_max  = 0.22h;
    const half  instability_fast_fall_alpha = 0.24h;
    const half  instability_fall_alpha      = 0.05h;
    const half  spatial_support_scale       = 0.75h;
    const half  spatial_support_blend       = 0.30h;
#if NSS_V1_MID_LOW_LUMA_DERIVATIVE
    const half moire_temporal_floor = 0.10h;
    const half moire_temporal_ceil  = 0.30h;
    const half moire_range_floor    = 0.99h;
    const half moire_range_ceil     = 0.999h;
    const half moire_range_scale    = 0.50h;
    const half flat_temporal_floor  = 0.015h;
    const half flat_temporal_ceil   = 0.030h;
    const half flat_range_floor     = 0.040h;
    const half flat_range_ceil      = 0.120h;
    const half flat_blue_floor      = 0.220h;
    const half flat_blue_ceil       = 0.300h;
    const half flat_luma_floor      = 0.450h;
    const half flat_luma_ceil       = 1.050h;
    const half flat_rgb_b_floor     = 0.800h;
    const half flat_rgb_b_ceil      = 1.400h;
    const half flat_flicker_scale   = 0.75h;
#endif  // NSS_V1_MID_LOW_LUMA_DERIVATIVE

    half4 deriv_tm1           = DecodeDerivativeStateFromStorage(deriv_tm1_h);
    half  raw_zero_state      = half(dot(abs(deriv_tm1_h), ((half4)(1.0h))) < half(1e-4f));
    half  decoded_zero_state  = half(dot(abs(deriv_tm1), ((half4)(1.0h))) < half(1e-4f));
    ...
    half  d_center            = ComputeDerivativeDelta(ycocg_c, half4(deriv_tm1.xyz, 0.0h), delta_weight);
    ...
    half  spatial_delta_sum   = dot(spatial_deltas, ((half4)(1.0h)));
    ...
    half     d_tn                  = ComputeDerivativeDelta(ycocg_tn, half4(deriv_tm1_n.xyz, 0.0h), delta_weight);
    half     d_ts                  = ComputeDerivativeDelta(ycocg_ts, half4(deriv_tm1_s.xyz, 0.0h), delta_weight);
    half     d_te                  = ComputeDerivativeDelta(ycocg_te, half4(deriv_tm1_e.xyz, 0.0h), delta_weight);
    half     d_tw                  = ComputeDerivativeDelta(ycocg_tw, half4(deriv_tm1_w.xyz, 0.0h), delta_weight);
    ...
    half spatial_support       = max(spatial_delta_sum - spatial_delta_max, half(0.0h)) * (half(1.0h) / half(3.0h));
    ...
    half flat_range_gate      = 1.0h - saturate((current_moire_range - flat_range_floor) * rcp(flat_range_ceil - flat_range_floor));
    half flat_blue_bias       = (-0.75h * ycocg_c.y) - (0.5h * ycocg_c.z);
    half flat_blue_gate       = saturate((flat_blue_bias - flat_blue_floor) * rcp(flat_blue_ceil - flat_blue_floor));
    half flat_luma_gate       = saturate((ycocg_c.x - flat_luma_floor) * rcp(0.10h)) * (1.0h - saturate((ycocg_c.x - flat_luma_ceil) * rcp(0.20h)));
    half flat_rgb_b           = ycocg_c.x - (0.5h * (ycocg_c.y + ycocg_c.z));
    half flat_rgb_b_gate      = saturate((flat_rgb_b - flat_rgb_b_floor) * rcp(0.10h)) * (1.0h - saturate((flat_rgb_b - flat_rgb_b_ceil) * rcp(0.20h)));
    ...
    half recall_excursion = max(supported_instability - prev_instability, half(0.0h));
    ...
    half output_rise_alpha   = 0.75h;
    half output_fall_alpha   = 0.80h;
    ...
    const half mid_low_entry_output_alpha = 0.25h;
    ...
    instability *= 1.0h - reset_history;

    half4 reset_state = half4(ycocg_c.xyz, 0.0h);
...
half4 CalculateLumaDerivative(half3 unjittered_colour, half4 deriv_tm1, half disocclusion_mask, out half instability)
{
    ...
    half applied_d_alpha = mix(kDerivAlpha, kDerivAlpha * 0.1h, clamp(luma_derivative_tm1, 0.0h, kDerivMax) * rcp(kDerivMax));
    ...
    return half4(luma_derivative, luma_t, 0.0h, 0.0h);
}
#endif  // NSS_YCOCG_LUMA_DERIVATIVE
...
{
    return (WarpHistory(lane_00.reproj_uv) + WarpHistory(lane_10.reproj_uv) + WarpHistory(lane_01.reproj_uv) + WarpHistory(lane_11.reproj_uv)) * 0.25h;
}
