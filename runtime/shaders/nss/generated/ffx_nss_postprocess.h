// Generated by runtime/tools/port_nss_shaders.py from Arm's ffx_nss_postprocess.h (MIT).
...
// Constants
// ---------------------------------------------------------------------------
static const float kEps_Post          = 1e-7;
...
static const int32_t kKpnChannels = 16;
#else
static const int32_t kKpnChannels = 36;
#endif
static const int32_t kKpnPackedChannelsPerTexel = 4;
static const int32_t kKpnPackedTexelsPerPixel   = kKpnChannels / kKpnPackedChannelsPerTexel;
static const int32_t kKpnPackedChannelShift     = 2;
static const int32_t kKpnPackedChannelMask      = kKpnPackedChannelsPerTexel - 1;

// QAT metadata for KPN coefficients (SINT) and temporal params (SNORM).
// QAT metadata (`_KpnCoefficients` / `_TemporalTensor` SINT).
static const half2 kKpnQuant = half2(0.003937007859349251, -127.0);
// Temporal params are sampled from SNORM image alias, so use SNORM dequant.
static const half2 kTemporalQuant = half2(0.49999999813735485, -1.0);

static const int32_t kLutPatternCount = 4;
...
#if !NSS_USE_SPARSE_2X2_FILTER
static const int16_t4 kTapDx0 = int16_t4(0, 0, 0, 2);
static const int16_t4 kTapDy0 = int16_t4(0, 2, 4, 0);
static const int16_t4 kTapCh0 = int16_t4(0, 2, 4, 12);
static const int16_t4 kTapDx1 = int16_t4(2, 2, 4, 4);
static const int16_t4 kTapDy1 = int16_t4(2, 4, 0, 2);
static const int16_t4 kTapCh1 = int16_t4(14, 16, 24, 26);
static const int16_t2 kTapD2  = int16_t2(4, 4);
static const int16_t  kTapCh2 = int16_t(28);

static const KernelPattern kKernelLut[kLutPatternCount] = {{int16_t2(-1, -1), int16_t(7), int16_t(0)},
                                                                                   {int16_t2(-2, -1), int16_t(1), int16_t(0)},
                                                                                   {int16_t2(-1, -2), int16_t(6), int16_t(0)},
                                                                                   {int16_t2(-2, -2), int16_t(0), int16_t(0)}};
#else
static const int16_t4 kTap2x2Dx[kLutPatternCount] = {int16_t4(-1, -1, +1, +1), int16_t4(+0, +0, +2, +2), int16_t4(-1, +1, -1, +1), int16_t4(+0, +0, +2, +2)};
static const int16_t4 kTap2x2Dy[kLutPatternCount] = {int16_t4(-1, +1, -1, +1), int16_t4(-1, +1, -1, +1), int16_t4(+0, +0, +2, +2), int16_t4(+0, +2, +0, +2)};
static const int16_t4 kTap2x2Ch[kLutPatternCount] = {int16_t4(0, 2, 8, 10), int16_t4(4, 6, 12, 14), int16_t4(1, 9, 3, 11), int16_t4(5, 7, 13, 15)};
#endif
...
half MaxHalf(half x)
{
    return min(x, half(65504.0h));
}

half3 MaxHalf(half3 x)
{
    return half3(min(x.r, half(65504.0h)), min(x.g, half(65504.0h)), min(x.b, half(65504.0h)));
}

half4 MaxHalf(half4 x)
{
    return half4(min(x.r, half(65504.0h)), min(x.g, half(65504.0h)), min(x.b, half(65504.0h)), min(x.a, half(65504.0h)));
}
...
half4 Tonemap4(half4 x)
{
    ...
    half4 xc = clamp(x, ((half4)(0.0h)), ((half4)(MAX_FP16)));
...
int8_t ReadKpnParamsInt8FromBase(int32_t kpn_texel_base_x, int32_t kpn_y, int32_t channel)
{
    ...
    int8_t4 vec        = UnpackI8x4(r_coefficients_kpn_buffer.Load(uint(linear_idx) * 4u));
...
half4 NormalizeSparseKernelWeights4(half4 weights)
{
    ...
    half  sum_w           = dot(clamped_weights, ((half4)(1.0h)));
...
half4 NormalizeSparseKernelWeights4(half4 weights, half4 valid_mask)
{
    ...
    half  sum_w           = max(dot(clamped_weights, ((half4)(1.0h))), half(MIN_SUMW_HALF));
    return ((half4)(clamped_weights * rcp(sum_w)));
}
...
void SampleTemporalParams(float2 uv, out half theta, out half alpha, out half gamma)
{
    ...
    half theta_in  = clamp(params.x, half(0.0h), half(1.0h));
    half theta_inv = half(1.0h) - theta_in;
...
Texture2D<float4> r_input_nearest_depth_coord : RDNU_REG_T(NSS_BIND_SRV_NEAREST_DEPTH_COORD);
...
Texture2D<uint4> r_offset_lut : RDNU_REG_T(NSS_BIND_SRV_OFFSET_LUT);
...
RWTexture2D<float4> rw_upscaled_output : RDNU_REG_U(NSS_BIND_UAV_UPSCALED_OUTPUT);
...
RWTexture2D<float4> rw_prev_upscaled_color : RDNU_REG_U(NSS_BIND_UAV_HISTORY_UPSCALED_COLOR);
...
{
    return half4(LoadColourTap(lr_tap), 1.0h);
}
...
{
    return half4(textureSample(_HistoryTex, uv).rgb, 1.0h);
}
...
half4 LoadWarpedHistoryCatmullPost(float2 uv)
{
    ...
    half2 w0  = f2 - 0.5h * (f3 + f);
    half2 w3  = 0.5h * (f3 - f2);
    half2 w2  = 0.5h * f + f2 * (2.0h - 1.5h * f);
    half2 w12 = 1.0h + 0.5h * f - 0.5h * f2;
    ...
    half4 left   = half4(textureSample(_HistoryTex, float2(left_x, center_uv.y)).rgb, 1.0h);
    half4 up     = half4(textureSample(_HistoryTex, float2(center_uv.x, up_y)).rgb, 1.0h);
    half4 center = half4(textureSample(_HistoryTex, center_uv).rgb, 1.0h);
    half4 right  = half4(textureSample(_HistoryTex, float2(right_x, center_uv.y)).rgb, 1.0h);
    half4 down   = half4(textureSample(_HistoryTex, float2(center_uv.x, down_y)).rgb, 1.0h);
    ...
    color       = any(lessThan(color, ((half3)(0.0h)))) ? clamp(color, cmin4.rgb, cmax4.rgb) : color;
...
void NormalizeFilterMoments(half4 accum_m1, half4 accum_m2, out half4 m1, out half4 m2)
{
    ...
    m1         = half4(accum_m1.rgb * rcp(denom), 0.0h);
    m2         = half4(accum_m2.rgb * rcp(denom), 0.0h);
}
...
static const int32_t kLutGroupCount = 1;
#else
static const int32_t kLutGroupCount = 3;
#endif
...
void FilterColour(int32_t2 output_px, out half4 m1, out half4 m2, out half4 center_sample)
{
    ...
    half4 accum_m1 = ((half4)(0.0h));
    half4 accum_m2 = ((half4)(0.0h));
    center_sample  = ((half4)(0.0h));
...
void FilterColour(int32_t2 output_px, out half4 m1, out half4 m2, out half4 center_sample)
{
    ...
    accum_m1 += mul(weights, taps);
    accum_m2 += mul(weights, NSS_SQ_MAT(taps));
...
void FilterColour(int32_t2 output_px, out half4 m1, out half4 m2, out half4 center_sample)
{
    ...
    center_sample = (lut_idx == int32_t(3)) ? taps[0] : ((half4)(0.0h));
    accum_m1 += mul(weights, taps);
    accum_m2 += mul(weights, NSS_SQ_MAT(taps));
#else
    ...
    accum_m1 += mul(w0, taps0);
    accum_m2 += mul(w0, NSS_SQ_MAT(taps0));
    ...
    center_sample = (lut_idx == int32_t(3)) ? taps1[0] : ((half4)(0.0h));
    accum_m1 += mul(w1, taps1);
    accum_m2 += mul(w1, NSS_SQ_MAT(taps1));
...
{
    half3 clamped = clamp(accumulated, ((half3)(0.0h)), ((half3)(1.0h - EPS)));
