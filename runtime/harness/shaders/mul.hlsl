// mul.hlsl - elementwise multiply of two equal-shape NCHW tensors: Out = A * B.
// Used for the DFM gating: gelu(g) * merge(cat[x, c]). Pointwise; no weights.
...
[numthreads(8, 8, 1)]
void mul_CS(uint3 tid : SV_DispatchThreadID)
{
    uint ox = tid.x;
    uint oy = tid.y;
    uint oc = tid.z;
